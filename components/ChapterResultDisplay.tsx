      ...
      <div className="flex flex-col gap-6">
        ...
        {result.imageUrl ? (
          <div className={`relative w-full aspect-video rounded-lg overflow-hidden shadow-2xl border border-stone-800 group no-print transition-all duration-700 ${isFocusMode ? 'shadow-black/50' : ''}`}>
            ...
            {/* Download Image Button Overlay */}
            <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
               <button 
                 onClick={onDownloadImage}
                 className="bg-stone-900/80 hover:bg-amber-700 text-white backdrop-blur-md p-2.5 rounded-full border border-stone-700 shadow-lg transition-colors"
                 title="Baixar Imagem"
               >
        ...
        <div id="chapter-print-area" className={`bg-stone-900 p-8 md:p-12 rounded-lg shadow-xl border border-stone-800 relative transition-all duration-500 ${isFocusMode ? 'border-transparent shadow-none bg-transparent' : ''}`}>
          ...
          {!isFocusMode && (
            <div className="absolute top-4 right-4 flex flex-wrap items-center justify-end gap-3 no-print z-10 pl-4">
              ...
              {/* Font Size Controls (Slider + Buttons) */}
              <div className="flex items-center bg-stone-950/80 rounded-md backdrop-blur-sm border border-stone-800 shadow-sm overflow-hidden mr-1 select-none p-1 gap-2">
                <button 
                ...
                <div className="flex items-center gap-2">
                  <input 
                    ...
                    className="w-20 h-1 bg-stone-700 rounded-lg appearance-none cursor-pointer accent-amber-500 hover:accent-amber-400"
          ...
          {/* 
            Typography Config:
            - We removed 'prose-lg' (18px) and now control size via inline style `fontSize`.
            - 'prose' (base) is kept for general spacing logic.
            - We add specific overrides to ensure the fontSize state cascades to paragraphs.
          */}
          <div 
            className={`prose prose-invert prose-p:font-serif prose-headings:font-serif prose-strong:text-amber-500 prose-p:text-justify print:prose-black print:text-black transition-all duration-500 ${isFocusMode ? 'leading-loose mx-auto max-w-3xl' : 'max-w-none'}`}
            style={{ fontSize: `${fontSize}px` }}
          >
             {/* H1 Title size is relative to base size, or we can enforce it. Tailwind prose scales headers. */}
             <h1 className="text-center mb-2 text-stone-100 print:text-black" style={{ fontSize: '2em', lineHeight: '1.2' }}>{result.textData.title}</h1>
